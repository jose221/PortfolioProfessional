...
import TokenPublicAPIUserComponent from "./TokenPublicAPIUserComponent";
let primary_url = window.url_api+"/admin/token-public-api-user";
let title = "Información de mis estudios"
class ListTokenPublicAPIUserComponent extends RComponent{
    constructor(props) {
        super(props);
    }
    ...
    processRowUpdate = (newRow, oldRow)=>{
        const changedField = Object.keys(newRow).find(field =>
            ...
            ['domain', 'mode', 'token'].includes(field)
        );
    ...
    render(){
        let columns = [
            ...
            {field: 'domain', headerName: 'Dominio', width: 200, type: 'string', description: 'Dominio del proyecto', sortable: true, editable: true},
            {field: 'mode', headerName: 'Modo', width: 200, type: 'string', description: 'Modo en el que esta, development o production', sortable: true, editable: true},
            {field: 'token', headerName: 'Token', width: 500, type: 'string', description: 'Llave para la api', sortable: false, editable: false},
        ];
        return (
            <div>
                <Backdrop
...
export default ListTokenPublicAPIUserComponent;
let name_component = document.querySelector("list-token-public-api-user-component");
if (name_component) {
    const propsContainer = name_component;
    const props = Object.assign({}, propsContainer.dataset);
    renderComponent(ListTokenPublicAPIUserComponent, name_component, props);
}
