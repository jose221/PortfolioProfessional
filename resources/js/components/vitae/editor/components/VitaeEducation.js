...
const VitaeEducation = ({ title, data, lang = 'es' }) => {
    const {
    ...
    const sectionTitle = safeString(title, lang === 'es' ? 'Educación' : 'Education');
    ...
    const renderEducationItem = (item, index) => {
        ...
        const degree = safeString(item.degree, "Título");
        ...
        return (
            <EducationItem key={item.id || index}>
        ...
        >
            ...
            <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)}>
                ...
                <DialogContent>
                    <Typography>
                        {lang === 'es' 
                            ? '¿Estás seguro de que deseas eliminar esta sección de educación?' 
                            : 'Are you sure you want to delete this education section?'
                        }
                    </Typography>
                </DialogContent>
                <DialogActions>
            ...
            <Dialog open={editDialogOpen} onClose={() => setEditDialogOpen(false)} fullWidth maxWidth="sm">
                <DialogTitle>
                    {lang === 'es' ? 'Editar Educación' : 'Edit Education'}
                </DialogTitle>
                <DialogContent>
                    {editingItem && (
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
                            <TextField
                                label={lang === 'es' ? 'Título/Grado' : 'Degree/Title'}
